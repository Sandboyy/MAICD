...
<!doctype html>
 ...
 <nav class="navbar navbar-expand-lg navbar-dark fixed-top" style ="background-color:#8B0000">
    ...
    <div class="text-center">
		...
		<?php 
		//encontrando os pacientes que estão doentes a mais de 20 dias
	/*if (file_get_contents('ultima_verificacao.txt') < time()) {
		file_put_contents('ultima_verificacao.txt', strtotime('+1 day', time()));
				
		$select_data = "SELECT paciente.pk_paci,
							   paciente.data_entr_paci,
							   DATE_ADD(paciente.data_entr_paci, INTERVAL 20 DAY) as 20dia
						FROM paciente";
		
		$resultado_select_data = mysqli_query($conexao, $select_data);	
		
		while ($row_data = mysqli_fetch_assoc($resultado_select_data)){
				
				$data = $row_data['data_entr_paci'];
				$dia20 = $row_data['20dia'];
				
				if($dia20 >= $data){
				deleta($row_data['pk_paci']);
				}
		}
	}*/	
		...
		$select_exibe_paciente_1 ="SELECT paciente.pk_paci,
						paciente.nome_paci, 
						paciente.cpf_paci, 
						paciente.fone_paci, 
						paciente.data_entr_paci, 
		...
		$resultado_select_exibe_paciente_1 = mysqli_query($conexao, $select_exibe_paciente_1);
				
		
		//numero de linhas para fazer a paginacao
		$total_linhas_paciente_1 = mysqli_num_rows($resultado_select_exibe_paciente_1);
		
		//setando quantos registros haverão em cada página
		$qtde_pag = 10;
		
		//calcular a quantidade de páginas necessárias
		$num_pag = ceil($total_linhas_paciente_1/$qtde_pag);
		...
		//contar os registros
		$total_linhas_paciente = mysqli_num_rows($resultado_select_exibe_paciente);	
	...
	<table class="table table-hover">
		...
		<tbody>
			...
			<tr>
				...
				<td><a href = "dados_paciente.php?unknow='.<?php echo $row_markers['pk_paci']?>.'" class="btn btn-outline-danger">GERENCIAR</a></td>
			</tr>
					<?php
		...
		<tr><td><td><td><td><td>
				<nav aria-label="Page navigation example">
					<ul class="pagination pagination-sm justify-content-center">
						...
						<?php 
						if($pag_atras != 0){?>
							<a class="page-link" href="lista_dados.php?pag=<?php echo $pag_atras;?>" aria-label="Previous">
						...
						<li class="page-item">
							<?php 
						//coloquei pag_adiante == 0 e funcionou, no tutorial era != 0, se der erro no futuro a cagada pode estar aqui
						if($pag_adiante != 0){?>
							<a class="page-link" href="lista_dados.php?pag=<?php echo $pag_adiante;?>" aria-label="Next">
	...
	<table style = "margin-bottom: 70px;">
		<tr>
			<a href = "form_paciente.php" class="btn btn-outline-danger">INSERIR PACIENTE</a></br>	
		</tr>
	</table>	
	</div>	
	</main>
